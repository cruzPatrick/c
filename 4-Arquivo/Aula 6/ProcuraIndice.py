...
while True:
    ...
    chave, posicao, prox = indexStruct.unpack(fi.read(indexStruct.size))
    if chave == cepProcurado:
        f.seek(posicao*dataStruct.size,os.SEEK_SET)
        logradouro, bairro, cidade, estado, sigla, cep, lixo = dataStruct.unpack(f.read(dataStruct.size))
        print(logradouro.decode('latin1'))
        print(bairro.decode('latin1'))
        print(i)
        break
    offset = prox
    if offset == 0:
